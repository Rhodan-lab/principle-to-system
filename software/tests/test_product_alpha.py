...
class ProductAlphaTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        source_root = MODULE_PATH.parents[2] / "software" / "product_alpha"
        target_root = self.root / "software" / "product_alpha"
        (target_root / "routes").mkdir(parents=True)
        for path in (source_root / "routes").iterdir():
            (target_root / "routes" / path.name).write_bytes(path.read_bytes())
        (target_root / "index.html").write_bytes((source_root / "index.html").read_bytes())
        ...
        documents = {
            ...
            "failure-atlas/feedback-instability.md": {"title": "Feedback Instability", "sections": {"4. Amplifying mechanism": "Compare observed cycling with the intended control band."}},
            "investigations/room-cooling.md": {"title": "Room Cooling", "sections": {"10. Model revision": "Compare heat transfer models."}},
            "design-challenges/passive-cooler.md": {"title": "Passive Cooler", "sections": {"3. Requirements and success measures": "Design under thermal constraints."}}
        }
    ...
    def test_static_assets_have_no_remote_dependencies(self) -> None:
        asset = self.root / "software" / "product_alpha" / "index.html"
        combined = asset.read_text(encoding="utf-8")
