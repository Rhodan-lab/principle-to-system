#!/usr/bin/env python3
"""Apply and verify direct sources used by the Phase 7 review."""
...
REGISTRY = ROOT / "sources" / "phase-7-reviewed-sources.json"
LEDGER = ROOT / "sources" / "source-ledger.md"
REPORT = ROOT / "reports" / "phase-7-physical-science-sources.json"
REQUIRED_FIELDS = (
    "title",
    "author",
    "date",
...
def load_registry() -> list[dict[str, str]]:
    ...
    if not isinstance(raw, list):
        raise ValueError("Phase 7 source registry must be a list.")
    ...
    if len(locators) != len(set(locators)):
        raise ValueError("Phase 7 source registry contains duplicate locators.")
    return records
...
def main() -> int:
    ...
    try:
        ...
        errors = verify(records) if args.write or args.check else []
    ...
    if errors:
        ...
        return 1
    print(f"Phase 7 source validation passed for {len(records)} records.")
    return 0
