#!/usr/bin/env python3
"""Generate deterministic Phase 46 population-readiness assurance evidence."""
...
CHECK_IDS = ['source-candidate-sha-exact', 'source-postmerge-sha-exact', 'source-finalization-commit-exact', 'source-phase-exact', 'source-state-validated', 'source-decision-exact', 'source-next-gate-exact', 'source-workflow-count-exact', 'source-policy-count-exact', 'source-profile-count-exact', 'source-record-count-exact', 'source-check-count-exact', 'source-failed-check-count-zero', 'source-slot-count-exact', 'source-populated-slot-count-zero', 'source-stage-count-exact', 'source-active-stage-count-zero', 'source-requirement-count-exact', 'source-evaluated-requirement-count-zero', 'source-human-gates-pending-exact', 'source-human-gates-satisfied-zero', 'assurance-policy-binding-exact', 'assurance-profile-binding-exact', 'assurance-record-sequence-exact', 'assurance-check-set-exact', 'assurance-check-count-exact', 'assurance-failed-check-count-zero', 'population-slots-preserved', 'population-slots-empty', 'population-slots-blocked', 'population-references-symbolic', 'population-values-absent', 'population-permission-false', 'population-stages-preserved', 'population-stages-inactive', 'population-stage-activation-false', 'population-requirements-preserved', 'population-requirements-unevaluated', 'population-requirement-evaluation-false', 'human-gates-preserved', 'human-gates-pending', 'human-gates-unsatisfied', 'dual-control-required', 'role-independence-required', 'candidate-identity-absent', 'candidate-body-absent', 'candidate-signature-absent', 'candidate-creation-forbidden', 'candidate-assembly-forbidden', 'candidate-population-forbidden', 'candidate-persistence-forbidden', 'candidate-submission-forbidden', 'population-run-absent', 'decision-selection-absent', 'decision-recording-absent', 'authorization-grant-absent', 'token-issuance-absent', 'execution-ticket-absent', 'execution-run-absent', 'response-envelope-absent', 'validation-result-absent', 'reviewer-identity-absent', 'reviewer-contact-forbidden', 'audit-event-absent', 'status-change-absent', 'atlas-boundary-preserved', 'external-network-boundary-preserved', 'repository-mutation-boundary-preserved', 'automatic-status-boundary-preserved', 'live-false', 'authority-separated', 'source-record-ledger-bound', 'assurance-ledger-chain-valid', 'assurance-verdict-exact', 'recovery-matrix-complete', 'mutation-rejection-boundary', 'no-inherited-status', 'local-only', 'real-authorization-false', 'source-authority-exact']
...
def build_document() -> dict[str, Any]:
    policy = signed({
        'id': 'principia-phase46-population-readiness-assurance-policy',
        'version': '0.1',
    ...
    for sequence in (1, 2):
        profile = signed({
            'id': f'principia:phase46-population-readiness-assurance-profile:{sequence:04d}',
            'sequence': sequence,
            'source_population_readiness_sequence': sequence,
        ...
        record = signed({
            'id': f'principia:phase46-population-readiness-assurance-record:{sequence:04d}',
            'sequence': sequence,
            'source_population_readiness_sequence': sequence,
            'policy_sha256': policy['sha256'],
            ...
            'population_stage_count': 18,
            'active_stage_count': 0,
            'population_requirement_count': 36,
            'evaluated_requirement_count': 0,
            ...
            'local_only': True,
            'status': 'population-readiness-assured-no-candidate',
            'verdict': DECISION,
        })
    ...
    result = {
        ...
        'population_assurance_check_count': len(CHECK_IDS) * 2,
        'failed_population_assurance_check_count': 0,
        'source_population_check_count': 144,
        'source_failed_population_check_count': 0,
        'population_slot_count': 36,
        'populated_slot_count': 0,
        'blocked_slot_count': 36,
        'symbolic_reference_count': 36,
    ...
    return {
        ...
        'population_assurance_policy': policy,
        'population_assurance_profiles': profiles,
        'population_assurance_records': records,
        'ledger': {'entries': entries, 'head_sequence': 2, 'head_sha256': previous},
        'recovery_matrix': {
            'scenario_count': 198,
            'baseline_count': 1,
            'mutation_count': 197,
            'rejected_mutation_count': 197,
            'categories': [
                'source-provenance','population-slot-integrity','population-stage-integrity',
...
def render_report(document: dict[str, Any], manifest_sha: str) -> str:
    ...
    lines = [
        '# Phase 46 — Offline Candidate Population Readiness Assurance', '',
        '> Date: 2026-07-31',
        '> Repository: `Rhodan-lab/principle-to-system`',
        f"> State: `{document['state']}`", '',
        '## Purpose', '',
        'Phase 46 independently assures the immutable Phase 45 population-readiness evidence for a still-uncreated authorization-decision candidate. It verifies provenance, frozen slot and stage states, unevaluated requirements, pending human gates, role separation, ledger integrity, and the complete absence of operational authority.', '',
        '## Immutable source boundary', '',
        f'- Phase 45 candidate SHA-256: `{SOURCE_CANDIDATE_SHA}`',
        f'- Phase 45 post-merge SHA-256: `{SOURCE_POSTMERGE_SHA}`',
        f'- Phase 45 authoritative finalization: `{SOURCE_FINALIZATION_COMMIT}`',
        '- Phase 45 applicable workflows: `38`',
        f'- Phase 46 candidate SHA-256: `{manifest_sha}`', '',
        '## Deterministic assurance result', '',
        f"- Population-assurance policies: `{result['population_assurance_policy_count']}`",
        f"- Population-assurance profiles: `{result['population_assurance_profile_count']}`",
        f"- Population-assurance records: `{result['population_assurance_record_count']}`",
        f"- Assurance checks: `{result['population_assurance_check_count']}`",
        f"- Failed assurance checks: `{result['failed_population_assurance_check_count']}`",
        f"- Population slots: `{result['population_slot_count']}`; populated: `0`; blocked: `{result['blocked_slot_count']}`",
        f"- Symbolic unresolved source references: `{result['symbolic_reference_count']}`",
        f"- Population stages: `{result['population_stage_count']}`; active: `0`",
        f"- Population requirements: `{result['population_requirement_count']}`; evaluated: `0`",
...
def write_or_check(check: bool) -> int:
    ...
    if failures:
        print('Phase 46 deterministic byte drift: ' + ', '.join(failures), file=sys.stderr)
        return 1
    print(f'Phase 46 deterministic bytes passed: sha256={manifest_sha}')
    return 0
