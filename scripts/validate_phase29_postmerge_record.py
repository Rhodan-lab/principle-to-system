...
def fail(errors: list[str]) -> int:
    print("Phase 29 post-merge record errors:", file=sys.stderr)
...
def main() -> int:
    ...
    expected = {
        "contract": "principia-offline-consequence-plan-review-response-intake-envelope-readiness-finalization/0.1",
        "phase": 29,
        ...
        "live": False,
        "next_gate": "offline-consequence-plan-review-response-intake-envelope-readiness-assurance-candidate",
    ...
    if finalization.get("candidate_record") != {
        "path": "release/phase-29-offline-consequence-plan-review-response-intake-envelope-readiness.json",
        "sha256": EXPECTED_SHA,
    }:
    ...
    if not isinstance(authority, Mapping):
        ...
        for key in (
            "atlas_call_permitted", "automatic_release_action", "automatic_status_change",
            "external_delivery_permitted", "external_network_required",
            "human_authorization_claimed", "repository_mutation",
            ...
            "response_receipt_permitted", "response_validation_authorized",
            "review_execution_authorized", "review_request_dispatch_authorized",
            "reviewer_contact_permitted",
        ):
            if authority.get(key) is not False:
                errors.append(f"authority {key}")
        if authority.get("local_response_envelope_readiness_permitted") is not True:
            errors.append("local envelope readiness disabled")
    ...
    for marker in (
        ...
        "Phase 29 state: **offline-consequence-plan-review-response-intake-envelope-readiness-validated**",
        "| 29 | Offline consequence-plan review-response intake envelope readiness | Merged and validated through PR #49 |",
        f"Phase 29 exact candidate validation passed at `{EXPECTED_HEAD}`",
        f"PR #49 was merged into `main` at commit `{EXPECTED_MERGE}`",
        ...
        "response-intake-envelope-readiness-recorded-no-response-received",
        "response-envelope-schema-ready-no-response",
        "envelope_readiness_record_count: 2",
        "integrity_rule_count: 20",
        "quarantine_reason_code_count: 20",
        "response_envelope_received_count: 0",
    ...
    for marker in (
        ...
        "Historical candidate report title:",
        "release/phase-29-postmerge.json",
        "2 envelope readiness records",
        "20 integrity rules",
        "20 quarantine reason codes",
        "87 deterministic scenarios",
        "response-intake-envelope-readiness-recorded-no-response-received",
        "> Live: `false`",
    ):
    ...
    if errors:
        return fail(errors)
    print(
        "Phase 29 post-merge record passed: exact candidate and PR pinned, "
        "two envelope readiness records finalized, zero envelopes or responses received."
    )
    return 0
