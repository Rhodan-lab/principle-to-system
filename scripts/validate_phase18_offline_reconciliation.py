...
def main() -> int:
    ...
    generated = reconciliation.build_outputs()
    errors.extend(reconciliation.check_outputs(generated))
    ...
    if not non_mutating(report.get("authority")):
        errors.append("reconciliation report must prohibit automatic mutations")
    if isinstance(report.get("authority"), Mapping) and report["authority"].get("status_inheritance") != "prohibited":
        errors.append("reconciliation report must prohibit status inheritance")

    records = report.get("records")
    if not isinstance(records, list) or len(records) != 2:
        errors.append("reconciliation report must contain exactly two records")
    else:
        expected_actions = ["revalidate", "block-release"]
        for index, record in enumerate(records):
            if not isinstance(record, Mapping):
                errors.append("reconciliation record is malformed")
                continue
            ...
            if record.get("required_action") != expected_actions[index]:
            ...
            for artifact in artifacts:
                if not isinstance(artifact, Mapping):
                    errors.append("reconciled artifact is malformed")
                    continue
                ...
                if artifact.get("reconciliation_state") != "current":
                    errors.append("reconciled artifact must be current")
    ...
    if not isinstance(source, Mapping):
        errors.append("reconciliation source provenance is missing")
    else:
    ...
    if checkpoint.get("decision") != "reconciled-no-mutation" or checkpoint.get("live") is not False:
        errors.append("reconciliation checkpoint decision or live boundary is incorrect")
    ...
    if not non_mutating(checkpoint.get("authority")):
        errors.append("reconciliation checkpoint must prohibit automatic mutations")

    if recovery.get("contract") != reconciliation.RECOVERY_CONTRACT:
        errors.append("reconciliation recovery contract is incorrect")
    scenarios = recovery.get("scenarios")
    if not isinstance(scenarios, list):
        errors.append("reconciliation recovery scenarios are missing")
    else:
        observed = {
    ...
    if release_file.get("contract") != release_record.CONTRACT:
        errors.append("candidate release contract is incorrect")
    ...
    if release_file.get("live_activation_permitted") is not False:
        errors.append("candidate release must not permit live activation")
    if release_file.get("next_gate") != "offline-reconciliation-policy-candidate":
        errors.append("candidate next gate is incorrect")
    if not non_mutating(release_file.get("authority")):
        errors.append("candidate authority must prohibit automatic mutations")
    ...
    if not isinstance(validation, Mapping) or validation.get("status") != "pending" or validation.get("pull_request") is not None or validation.get("tested_head_commit") is not None:
        errors.append("immutable candidate record must remain pending without self-referential PR or head")
